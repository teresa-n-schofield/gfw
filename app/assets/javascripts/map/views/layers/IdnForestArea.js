...
define(
  ...
  (CartoDBLayerClass, IdnForestAreaCartocss) => {
    const IdnForestArea = CartoDBLayerClass.extend({
      options: {
        sql:
          "SELECT the_geom_webmercator, fungsikawa, legend_en as name, cartodb_id, '{tableName}' AS tablename, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
